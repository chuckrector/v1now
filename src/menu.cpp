...
drawloop:
  ...
  if (up)   { if (!whoisptr) whoisptr=numchars-1;
              else whoisptr--;
              playeffect(0);
              first=1;
            }
...
drawloop:
  ...
  if (b1)
  {
    if (!p) { playeffect(13); gp+=items[pstats[l].inv[ptr]].price/2;
...
drawloop:
  ...
  {dec_to_asciiz(magic[pstats[l].maginv[ptr]].cost/2,strbuf);a=strlen(strbuf);
   strbuf[a]=' ';strbuf[a+1]='G';strbuf[a+2]=0;
   PutMessageBox(strbuf);   }
  else PutMessageBox("");
...
//  PutEquipBox(partyidx[c]-1);
  ...
  if (b1)
  {
    if (!p) { playeffect(13); gp+=magic[pstats[l].maginv[ptr]].cost/2;
              RemoveMagic(l,ptr);}
    first=2;
  }
...
drawloop:
  ...
  {dec_to_asciiz(magic[storeinv[ptr]].cost,strbuf);a=strlen(strbuf);
   strbuf[a]=' ';strbuf[a+1]='G';strbuf[a+2]=0;
   PutMessageBox(strbuf);   }
  else PutMessageBox("");
 ...
 if (up)     { if (my) my--;
  ...
  if (b1)    {
               if (storeinv[ptr])
               {
                   if (gp<magic[storeinv[ptr]].cost) playeffect(12);
                   else BuyMagic(ptr);
               }
               first=1;
             }
