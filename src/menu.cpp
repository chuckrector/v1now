...
unsigned char menuptr[256];
unsigned char charptr[960];
unsigned char itmptr[576];
int atkp;
int defp;
int magp;
int mgrp;
...
PutCharBox(char a, char b, char c, char d, char e, char p)
{
  ...
  if (!a)
  {
    greyscale(16, 32, chrs, (unsigned char *)gsimg);
  ...
  if (numchars > 1)
  {
    if (!b)
    {
      greyscale(16, 32, chrs + 15360, (unsigned char *)gsimg);
  ...
  if (numchars > 2)
  {
    if (!c)
    {
      greyscale(16, 32, chrs + 30720, (unsigned char *)gsimg);
  ...
  if (numchars > 3)
  {
    if (!d)
    {
      greyscale(16, 32, chrs + 46080, (unsigned char *)gsimg);
  ...
  if (numchars > 4)
  {
    if (!e)
    {
      greyscale(16, 32, chrs + 61440, (unsigned char *)gsimg);
...
PutBuyCharBox(char ptr, char p)
{
  ...
  PutCharBox(
      equip[items[a].equipidx].equipable[partyidx[0] - 1],
      ...
      equip[items[a].equipidx].equipable[partyidx[4] - 1],
      p);
}
...
MPutBuyCharBox(char ptr, char p)
{
  ...
  PutCharBox(
      mequip[magic[a].equipidx].equipable[partyidx[0] - 1],
      ...
      mequip[magic[a].equipidx].equipable[partyidx[4] - 1],
      p);
}
...
drawloop:
  ...
  if (b1)
  {
    if (pstats[l].magcnt < 24)
    {
      ...
      if ((!mequip[magic[storeinv[ptr]].equipidx].equipable[l]) ||
          (alreadyhave))
      {
        playeffect(12);
      }
