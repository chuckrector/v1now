...
drawloop:
  ...
  if (up)   { if (!ptr) ptr=numchars-1;
              else ptr--;
              playeffect(0);
              first=1;
            }

  if (b1)   { l=partyidx[c]-1;
              t1=pstats[l].inv[p];
...
{ unsigned char l,i,a,*img;
  ...
  if (items[a].equipflag && equip[items[a].equipidx].equipable[l])
  {
     ...
     gotoxy(26,124);
     printstring((char*)"ATK "); dec_to_asciiz(pstats[l].atk,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),124); printstring(strbuf); printstring((char*)">");dec_to_asciiz(atkp,strbuf);
     if (pstats[l].atk<atkp) fontcolor(97); if (atkp<pstats[l].atk) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),124); printstring(strbuf); fontcolor(31); gotoxy(26,133);
     printstring((char*)"DEF "); dec_to_asciiz(pstats[l].def,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),133); printstring(strbuf); printstring((char*)">");dec_to_asciiz(defp,strbuf);
     if (pstats[l].def<defp) fontcolor(97); if (defp<pstats[l].def) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),133); printstring(strbuf); fontcolor(31);gotoxy(26,142);
     printstring((char*)"HIT "); dec_to_asciiz(pstats[l].hitc,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),142); printstring(strbuf); printstring((char*)">");dec_to_asciiz(hitp,strbuf);
     if (pstats[l].hitc<hitp) fontcolor(97); if (hitp<pstats[l].hitc) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),142); printstring(strbuf); fontcolor(31);gotoxy(26,151);
     printstring((char*)"DOD "); dec_to_asciiz(pstats[l].dodc,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),151); printstring(strbuf); printstring((char*)">");dec_to_asciiz(dodp,strbuf);
     if (pstats[l].dodc<dodp) fontcolor(97); if (dodp<pstats[l].dodc) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),151); printstring(strbuf); fontcolor(31);gotoxy(26,160);
     printstring((char*)"MAG "); dec_to_asciiz(pstats[l].magc,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),160); printstring(strbuf); printstring((char*)">");dec_to_asciiz(magp,strbuf);
     if (pstats[l].magc<magp) fontcolor(97); if (magp<pstats[l].magc) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),160); printstring(strbuf); fontcolor(31);gotoxy(26,169);
     printstring((char*)"MGR "); dec_to_asciiz(pstats[l].mgrc,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),169); printstring(strbuf); printstring((char*)">");dec_to_asciiz(mgrp,strbuf);
     if (pstats[l].mgrc<mgrp) fontcolor(97); if (mgrp<pstats[l].mgrc) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),169); printstring(strbuf); fontcolor(31);gotoxy(26,178);
     printstring((char*)"REA "); dec_to_asciiz(pstats[l].reac,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),178); printstring(strbuf); printstring((char*)">"); dec_to_asciiz(reap,strbuf);
     if (pstats[l].reac<reap) fontcolor(97); if (reap<pstats[l].reac) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),178); printstring(strbuf); fontcolor(31);gotoxy(26,187);
     printstring((char*)"FER "); dec_to_asciiz(pstats[l].ferc,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),187); printstring(strbuf); printstring((char*)">");dec_to_asciiz(ferp,strbuf);
     if (pstats[l].ferc<ferp) fontcolor(97); if (ferp<pstats[l].ferc) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),187); printstring(strbuf); fontcolor(31);gotoxy(26,196);
     printstring((char*)"MBL "); dec_to_asciiz(pstats[l].mblc,strbuf); gotoxy(80-(strlen((const char*)strbuf)*8),196); printstring(strbuf); printstring((char*)">");dec_to_asciiz(mblp,strbuf);
     if (pstats[l].mblc<mblp) fontcolor(97); if (mblp<pstats[l].mblc) fontcolor(36); gotoxy(110-(strlen((const char*)strbuf)*8),196); printstring(strbuf); fontcolor(31);
  }
...
{ int first=1, ptr=0,i,j;
  ...
  if (!magic[t1].useflag || magic[t1].useflag>4) return;
  if (numchars==1)
     { l=a; goto usesec; }
...
drawloop2:
  ...
  if (first==2) if (b1 || b2 || b4) goto drawloop2;
     else { an=0; timer_count=0; return; }
  if (first && !b1 && !b2 && !b4 && !down && !up) first=0;
  else if (first) goto drawloop2;


  while (!b4 && !b2 && !b1) goto drawloop2;
  while (b4 || b2 || b1) { first=2; goto drawloop2; }
  timer_count=0; an=0;

  }

  if (magic[t1].useflag==2 || magic[t1].useflag==4) return;
...
drawloop:
  ...
  gotoxy(45,123); printstring("Cast");
  tmenubox(20, 139, 115, 159);
  gotoxy(45,145); printstring("MP Cost");
  ...
  if (b1)   { switch (ptr)
              { case 0: MagicUse(c,p);
                ...
                case 1: if (numchars==1) break;
                        ItemGive(c,p);
                        first=2; break;
                case 2: if (!magic[pstats[l].maginv[p]].price)
                           { playeffect(12); first=2; break; }
                        if (p>5) RemoveItem(l,p);
                        else
                        { a=pstats[l].maginv[p];
