...
char layervcwrite = 1;
unsigned char *vcscreen, hookretrace, *vcscreen2,
    *vcscreen1;         // VClayer buffer ptr, rtr hook
...
int foregroundlock = 1, xwin1 = 0,
    ywin1 = 0; // front layer independent scrolling stuff
...
DrawLayer0(int xw, int yw)
{
  ...
  for (i = 0; i < 14; i++)
  {
    ...
    {
      img = vsp0 +
            (tileidx[map0[(((ytc + i) % ysize) * xsize) + ((xtc + j) % xsize)]]
             << 8);
