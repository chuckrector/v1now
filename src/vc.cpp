...
char *effectvc;
char *startupvc;          // Effect VC code buffer / Startup.vcs
unsigned char *vcdatabuf; // The data buffer for VC use
char *code;
char *basevc;                    // current code data ptr
unsigned int scriptofstbl[1024]; // Map script offset table
unsigned int effectofstbl[1024]; // Effect script offset table
unsigned int magicofstbl[1024];  // Magic script offset table - ric: 08/May/98 -
                                 // who forgot this then?
...
InitVCMem()
{
  ...
  vcdatabuf = (unsigned char *)valloc(vcbufm, "vcdatabuf");
}
...
LoadVC(FILE *f)
{
  ...
  mapvc =
      (char *)valloc(mapvcs, "LoadVC:mapvc"); // allocate necessary mapvc mem
