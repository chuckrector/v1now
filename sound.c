...
parseloop:
  if ((keyboard_map[SCAN_ALT]) && (keyboard_map[SCAN_CTRL]) &&
      (keyboard_map[SCAN_DEL])) err("Exiting: CTRL-ALT-DEL pressed.");
...
//       goto parseloop; }
  ...
  if (!strcmp(strbuf,"speeddemon"))
     { speed=1; goto parseloop; }
  if (!strcmp(strbuf,"meseta"))
     { moneycheat=1; goto parseloop; }
  if (!strcmp(strbuf,"vspspeed"))
     { vspspeed=1; goto parseloop; }
...
{ char t;
  ...
  kb1=28;  kb2=56;  kb3=1;  kb4=57;
  jb1=1;   jb2=2;   jb3=3;  jb4=4;
  waitvrt=0;        jf=0;
  vspspeed=0;
...
{
  if (md_device==3) return;
  if (!strcmp(sngnme,playingsong)) return;
